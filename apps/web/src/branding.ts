...
/**
 * LateShift Cloud brand override for served web clients. When set (any
 * non-desktop web build, or an explicit `VITE_LSC_BRAND_NAME`), the sidebar
 * shows the LateShift wordmark + logo instead of the upstream T3 wordmark. The
 * desktop app keeps its injected branding, so this is null there.
 */
export const LSC_BRAND_NAME =
  import.meta.env.VITE_LSC_BRAND_NAME?.trim() ||
  (injectedDesktopAppBranding ? null : "LateShift Cloud");
