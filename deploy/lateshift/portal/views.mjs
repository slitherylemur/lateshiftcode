  ...
  const items = rows
    .map((r, i) => {
      ...
      return `<li>
        ...
        <span class="who">${esc(r.label || r.name)}${you}</span>
...
export function renderDashboard(props) {
  const {
    ...
    github,
    workspaceUrl,
    // W3: pool remaining (provider truth) and share of consumption (our
    // attribution) are two distinct displays and are never blended.
    pool,
    consumption,
    myUsage,
    // The registry short name of the signed-in user. Used ONLY to mark the
    // "you" row of the leaderboard; it is never rendered (D12).
    consumptionHighlight = null,
  } = props;
  ...
  const body = `<main class="container">
    ...
    ${renderConsumption(consumption, { highlight: consumptionHighlight })}
